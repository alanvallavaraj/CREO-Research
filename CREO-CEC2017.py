# ============================================================
# OFFICIAL-STYLE CEC2017 + CREO-DE EXPERIMENT
# ============================================================
# Expected input:
...
#
# Notes:
# - Uses the uploaded cec17_python-master.zip package
...
ZIP_PATH = "cec17_python-master.zip"   # uploaded zip filename
EXTRACT_ROOT = "cec17_official"
PACKAGE_DIR = os.path.join(EXTRACT_ROOT, "cec17_python-master")
RESULTS_DIR = "cec17_results"
...
RUNS = 30
POP_SIZE = 50
FUNCTION_IDS = list(range(1, 31))   # F1-F30
...
if not os.path.exists(PACKAGE_DIR):
    raise FileNotFoundError(f"Expected package dir not found: {PACKAGE_DIR}")

print("Extracted CEC2017 package to:", PACKAGE_DIR)
...
# ============================================================
# CEC2017 here is unconstrained. To keep CREO meaningful as a
# repair-centric framework, we use:
#   - box-feasibility
#   - soft "stability" region around the bounded search domain
# This preserves a fairness story for repair/repair-guided search
# without changing the official objective values themselves.
#
# Objective values are ALWAYS evaluated by official CEC2017 code.
# ============================================================
...
def total_violation(x: np.ndarray) -> float:
    # Box violation is already clipped, but we define an additional
    # soft-region preference to create a meaningful repair setting.
    #
    # This does NOT alter the official objective evaluator.
    # It only affects feasibility-aware selection for CREO variants.
    #
    # Prefer staying inside [-80, 80] and keeping average norm moderate.
    v = 0.0
...
def repair_solution(x: np.ndarray) -> np.ndarray:
    ...
    # Shrink toward centre iteratively
    for _ in range(10):
        y = 0.80 * y
        y = clip_to_bounds(y)
        if total_violation(y) <= 1e-12:
...
def run_ga(func_id: int, dim: int, max_evals: int):
    ...
    while nfe < max_evals:
        next_pop = []
        elite_idx = min(range(len(evals)), key=lambda i: (1 - evals[i].feasible, evals[i].violation, evals[i].fitness))
...
def run_pso(func_id: int, dim: int, max_evals: int):
    ...
    gbest_idx = min(range(POP_SIZE), key=lambda i: (1 - evals[i].feasible, evals[i].violation, evals[i].fitness))
    ...
    while nfe < max_evals:
        ...
        best_idx = min(range(POP_SIZE), key=lambda i: (1 - pbest_eval[i].feasible, pbest_eval[i].violation, pbest_eval[i].fitness))
...
def run_gwo(func_id: int, dim: int, max_evals: int):
    ...
    while nfe < max_evals:
        order = sorted(range(POP_SIZE), key=lambda i: (1 - evals[i].feasible, evals[i].violation, evals[i].fitness))
...
def friedman_rank_table(score_matrix: Dict[str, List[float]]) -> pd.DataFrame:
    ...
    _stat, _p = friedmanchisquare(*arrays)
...
def main():
    ...
    for dim in DIMENSIONS:
        ...
        for func_id in FUNCTION_IDS:
            ...
            for algo in ALGORITHMS:
                ...
                try:
                    stat, p = wilcoxon(creo_scores, other_scores, zero_method="wilcox", alternative="two-sided")
                except ValueError:
    ...
    print("\nSaved:")
    print(os.path.abspath(os.path.join("..", "..", RESULTS_DIR, "all_benchmark_results.csv")))
    print(os.path.abspath(os.path.join("..", "..", RESULTS_DIR, "overall_algorithm_summary.csv")))
    print(os.path.abspath(os.path.join("..", "..", RESULTS_DIR, "wilcoxon_creo_de_vs_others.csv")))
    print(os.path.abspath(os.path.join("..", "..", RESULTS_DIR, "friedman_ranks.csv")))
    print(os.path.abspath(os.path.join("..", "..", RESULTS_DIR, "overall_friedman_mean_ranks.csv")))
