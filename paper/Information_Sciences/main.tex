...
% Manuscript prepared for submission to Information Sciences (Elsevier).
% Journal ISSN: 0020-0255.
% Elsevier CAS single-column manuscript template.
% Keep all submission files in this directory; Editorial Manager does not
% process LaTeX submissions that depend on subdirectories.
...
\hypersetup{
  hypertexnames=false,
  pdftitle={Repair-Guided Evolutionary Optimisation (CREO): A Structured Search Paradigm for Continuous Optimisation},
  pdfauthor={Alan Immanuel Benjamin Vallavaraj},
  pdfsubject={Manuscript prepared for submission to Information Sciences (ISSN 0020-0255)}
}

% Self-contained algorithm float and pseudocode commands. This avoids an
% external algorithm package that may not be installed by Editorial Manager.
\floatstyle{ruled}
...
\shorttitle{Repair-Guided Evolutionary Optimisation}
\shortauthors{A. I. B. Vallavaraj}

\title[mode=title]{Repair-Guided Evolutionary Optimisation (CREO):\\
A Structured Search Paradigm for Continuous Optimisation}

\author[1]{Alan Immanuel Benjamin Vallavaraj}
\cormark[1]
...
\begin{abstract}
Population-based metaheuristics are widely used for nonlinear, multimodal and derivative-free optimisation. Conventional repair operators restore feasibility but normally discard the displacement produced by repair. This paper introduces Repair-Guided Evolutionary Optimisation (CREO), which retains that displacement and reuses it as directional information. Its Differential Evolution instantiation, CREO-DE, combines differential mutation, attraction towards the best repaired candidate, repair-guided displacement and stochastic exploration.

CREO-DE is evaluated on 29 CEC2017 functions (F2 excluded) using 30 runs, dimensions $D=10$ and $D=30$, a population size of 50 and a budget of $10{,}000D$ objective evaluations. The experiments apply an author-defined preferred-region and feasibility-handling protocol. Differential Evolution achieves the best aggregate rank at both dimensions. CREO-DE is competitive on selected functions at $D=10$ but deteriorates at $D=30$. A matched ablation produces win/tie/loss counts of $12/0/17$ at $D=10$ and $1/0/28$ at $D=30$ against the non-guided variant.

The results show that repair displacement can influence search, but its usefulness is dimension-dependent. Future work should investigate adaptive repair weighting, parameter sensitivity and explicitly constrained optimisation problems.
\end{abstract}

\begin{highlights}
\item Repair-induced displacement is reused as an evolutionary search direction.
\item A matched ablation isolates the effect of the repair-guidance term.
\item Fixed repair guidance helps selected low-dimensional benchmark functions.
\item Performance deteriorates when dimensionality increases from 10 to 30.
\end{highlights}

\begin{keywords}
evolutionary computation \sep differential evolution \sep repair operators \sep
constraint handling \sep continuous optimisation \sep CEC2017 benchmark
\end{keywords}
...
During the preparation of this work, the author used OpenAI ChatGPT to assist with language editing, manuscript structure, and \LaTeX{} formatting. After using this tool, the author reviewed and edited the content as needed and takes full responsibility for the content of the publication.
